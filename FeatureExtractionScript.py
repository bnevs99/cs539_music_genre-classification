...
class GenreFeatureData:
    ...
    def __init__(self):
        ...
        if not os.path.exists("./gtzan"):
            self.split_data(self.dir_raw_Data, self.dir_all_files, self.dir_trainfolder, self.dir_testfolder, self.dir_devfolder)
        ...
        #Below line used for testing load_preprocess_data() method
    ...
    @staticmethod
    def split_data(dir_folder, dir_all, dir_train, dir_test, dir_dev):
        ...
        if not os.path.exists(dir_test):
            os.mkdir(dir_test)
        if not os.path.exists(dir_dev):
            os.mkdir(dir_dev)

        audio_files = dict()
        for dirName in os.listdir(dir_folder):
            audio_files[dirName] = []
            for file in os.listdir(dir_folder + "/" + dirName):
                if file.endswith(".au") or file.endswith(".wav"):
                    path = dir_folder + "/" + dirName + "/" + file
                    audio_files[dirName].append(path)
        
        for genre in audio_files:
            count = 0
            for file_path in audio_files[genre]:
                index = file_path.index('.', 6)
                file_name = file_path[index:]
                file_name_full = genre + file_name
                if count % 10 == 8:
                    os.rename(file_path, dir_test + "/" + file_name_full)
                elif count % 10 == 9:
                    os.rename(file_path, dir_dev + "/" + file_name_full)
                else:
                    os.rename(file_path, dir_train + "/" + file_name_full)
                count+=1
